...
function updateOTPInDatabase($pdo, $user_id) {
    ...
    $stmt = $pdo->prepare("UPDATE users SET hash_otp = ?, otp_expiry = ? WHERE user_id = ?");
...
if (isset($_POST['get_otp'])) {
    ...
    $stmt = $pdo->prepare("SELECT otp_expiry FROM users WHERE user_id = ?");
    $stmt->execute([$user_id]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($user) {
...
if (isset($_POST['login'])) {
    ...
    $stmt = $pdo->prepare("SELECT hash_otp, otp_expiry FROM users WHERE user_id = ?");
    $stmt->execute([$user_id]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        $otp_hash = $user['hash_otp'];
        ...
        if (password_verify($entered_otp, $otp_hash)) {
            ...
            echo "<script>alert('Welcome!'); window.location = 'candidate.php';</script>";
            exit();
        } else {
            // Invalid OTP
        ...
        echo "<script>alert('User not found. Please login again.'); window.location = 'index.php';</script>";
        exit();
    }
}
?>