...
$page_to_redirect_after_action = 'activate_accounts.php'; // Default page to reload for adjustments
$page_for_final_success = 'index.php'; // Page to go to after successful activation (e.g., main user list)
...
$club_memberships = [];
try {
    // Fetching predefined clubs and any 'Other' typed values, excluding 'None' for direct filtering if needed
    // You might want to adjust this query based on how you store "Other" club names
...
// --- Start: Handle Conditional Activation ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['activate_all_flag'])) {
    ...
    if (!empty($conditions)) {
        ...
        try {
            ...
            if ($affected_rows > 0) {
                $message = "Successfully activated " . $affected_rows . " user account(s) based on your criteria.";
                echo "<script>
                    alert('" . htmlspecialchars($message) . "');
                    window.location = '" . $page_for_final_success . "';
                </script>";
            } else {
                $message = "No user accounts matched your criteria or they were already active.";
                echo "<script>
                    alert('" . htmlspecialchars($message) . "');
            ...
            $message = "Error activating accounts: " . $e->getMessage();
            echo "<script>
        ...
        $message = "Please select at least one specific filter criterion (gender, batch, department, club, or representative status) to activate accounts, or use the 'Activate ALL Users' button."; // <-- MODIFIED
         echo "<script>
            alert('" . htmlspecialchars($message) . "');
            window.location = '" . $page_to_redirect_after_action . "';
        </script>";
...
// --- End: Handle Conditional Activation ---


// --- Start: Handle "Activate All" Flag ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['activate_all_flag']) && $_POST['activate_all_flag'] === 'true') {
    try {
        $stmt = $pdo->prepare("UPDATE users SET account = 'Active'");
        $stmt->execute();
        echo "<script>
            alert('All user accounts have been activated successfully!');
            window.location = '" . $page_for_final_success . "';
        </script>";
    } catch (PDOException $e) {
        echo "<script>
            alert('Error activating all accounts: " . htmlspecialchars($e->getMessage()) . "');
            window.location = '" . $page_for_final_success . "';
        </script>";
    }
    exit;
}
// --- End: Handle "Activate All" Flag ---
...
<head>
    ...
    <title>Activate User Accounts</title>
...
<div class="container">
    ...
    <p class="instructions">Select criteria to activate user accounts. If multiple filters are selected, users must match ALL criteria.</p>
    ...
    <form action="activate_accounts.php" method="POST">
        ...
        <div class="form-group">
            ...
            <select id="club_membership_filter_select" name="club_membership_filter">
                ...
                <?php foreach ($club_memberships as $club): // Display specific clubs fetched from DB ?>
    ...
    <button type="button" class="activate-all" onclick="activateAllUsers()">Activate ALL User Accounts</button>
</div>

<script>
function activateAllUsers() {
    if (confirm('Are you sure you want to activate ALL user accounts? This cannot be undone easily.')) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'activate_accounts.php';

        const hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden';
        hiddenInput.name = 'activate_all_flag';
